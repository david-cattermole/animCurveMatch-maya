...
#ifndef MAYA_CURVE_FIT_UTILS_H
#define MAYA_CURVE_FIT_UTILS_H
...
void curveFunc(double *p, double *x, int m, int n, void *data) {
    ...
    for (i = 0; i < (m / 3); ++i) {
        ...
        MAngle oa(it, degUnit);

        userData->dstCurveFn->setValue(i, v);
        userData->dstCurveFn->setAngle(i, ia, true);
        userData->dstCurveFn->setAngle(i, oa, false);
    }
...
bool solveCurveFit(int iterMax,
    ...
    for (i = 0; i < (m / 3); ++i) {
        double v = dstCurveFn->value(i);
        MAngle ia = 0;
        MAngle oa = 0;
        double iw = 1.0;
        double ow = 1.0;
        dstCurveFn->getTangent(i, ia, iw, true);
        dstCurveFn->getTangent(i, oa, ow, true);
